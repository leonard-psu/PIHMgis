...
int flood(char *demfile, char *pointfile, char *newfile)
{
  ...
  double bndbox[4],csize;    These are external now */
        ...
        if(dn == nullptr || is == nullptr || js == nullptr || ipool == nullptr || jpool == nullptr)
        {
            qDebug() << "Error: flood  Cannot allocate memory for stacks";
...
int setdf(float mval)
{
    ...
    try {
        ...
        int i,j,k,n,nflat,ni,ip,imin,jn,in,np1,nt = 0;
        float fact[9],per=1.;
        ...
        if(apool[j][i]<=0)   /* not already part of a pool  */
        {
                ...
                /* not on boundary  */
            {
                ...
                if(npool >= pstack)
                {
                    ...
                    {
                        /*  Try enlarging   */
                        qDebug() << "INFO: Enlarging pool stack";
                        printf("\n Enlarging pool stack\n");

                        pstack=(int) (pstack + nx*ny*.1);
                        if(pstack > nx*ny)
                        {
                            qDebug() << "ERROR: Pool stack too large, exiting ...";
                        ...
                        {
                            qDebug() << "ERROR: Cannot reallocate pool stack, exiting ...";
                    ...
                    {
                        qDebug() << "ERROR: Could not enlarge Pool stack ...";
...
int addstack(int i, int j)
{
    ...
    try {
        ...
        nis=nis+1; //What happens with bad nis? TODO
        ...
        if(nis >= istack )
        {
            ...
            if(istack > nx*ny)
            {
                qDebug() << "Error: is,js stack too large, exiting";
            ...
            if(is == nullptr || js == nullptr || dn == nullptr)
            {
                qDebug() << "Error: Could not enlarge stack\n";
